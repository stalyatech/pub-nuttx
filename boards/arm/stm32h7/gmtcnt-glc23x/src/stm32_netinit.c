...
#define STM32H7_ETHMAC_DEVNAME	"eth0"
  ...
  if (enable && g_ethmac_handler != NULL)
    {
      ...
      stm32_gpiosetevent(GPIO_ETH_IRQ, false, true, true,
  ...
  else
    {
      ...
      stm32_gpiosetevent(GPIO_ETH_IRQ, false, false, false,
                         NULL, NULL);
    }
}
#endif
                 ...
                 phy_enable_t *enable)
{
  ...
  ninfo("%s: handler=%p\n", intf, handler);
  phyinfo("ETHMAC: devname=%s\n", STM32H7_ETHMAC_DEVNAME);

  DEBUGASSERT(intf);

  flags = enter_critical_section();

  if (strcmp(intf, STM32H7_ETHMAC_DEVNAME) == 0)
    {
