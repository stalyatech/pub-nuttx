/****************************************************************************
 * boards/arm/stm32h7/gmtcnt-glc23x/src/stm32_w25.c
 *
...
#define HAVE_W25  1
#if !defined(CONFIG_STM32H7_SPI3) || !defined(CONFIG_MTD_W25)
#  undef HAVE_W25
#endif
...
#if CONFIG_GMTCNT_GLC23X_SPIFLASH_NPARTMAIN == 0
...
{
  struct mtd_dev_s *part[CONFIG_GMTCNT_GLC23X_SPIFLASH_NPARTMAIN]={0};
  char blockname[32];
  char charname[32];
  char mntpoint[32];
  int ret, i;
  ...
  for (i = 0;
       i < CONFIG_GMTCNT_GLC23X_SPIFLASH_NPARTMAIN;
       offset += nblocks, i++)
    {
 ...
 *
 ****************************************************************************/
#if CONFIG_GMTCNT_GLC23X_SPIFLASH_NPARTRESV > 0
static int create_resv_partitions(struct mtd_dev_s *mtd, off_t offset, off_t nblocks)
{
  struct mtd_dev_s *part[CONFIG_GMTCNT_GLC23X_SPIFLASH_NPARTRESV]={0};
  char blockname[32];
  char charname[32];
  int ret, i;
  ...
  for (i = 0;
       i < CONFIG_GMTCNT_GLC23X_SPIFLASH_NPARTRESV;
       offset += nblocks, i++)
    {
      syslog(LOG_INFO, "INFO: Partition %d. Block offset=%lu, size=%lu\n",
                       i, (unsigned long)offset, (unsigned long)nblocks);
...
#endif /* CONFIG_GMTCNT_GLC23X_SPIFLASH_NPARTRESV > 0 */
...
int stm32_w25initialize(int minor)
{
  ...
  ret = create_resv_partitions(mtd,
                               offset,
                               blkreserved / CONFIG_GMTCNT_GLC23X_SPIFLASH_NPARTRESV *
                               blkpererase);
  if (ret < 0)
    {
      return ret;
